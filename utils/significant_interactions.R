...
# Load datasets
load(paste(codeDir,"/oncopanel_genes_WD_v1.4.Rdata", sep = ""))
load(paste(codeDir, "/all_genes.Rdata", sep = ""))

# Load scripts with additional functions for analysis
#source("./cHiC_analysis.R", chdir = FALSE)
#source("./cHiC_plots.R", chdir = FALSE)
#source("./cHiC_fishertest.R", chdir = FALSE)
...
for (i in seq(1,length(all_genes_files),1)){
	...
	if (nrow(gene_pvals_sig) != 0){
		...
		plot_genome <- genomeManhattan(df, gene = focal.gene, 
