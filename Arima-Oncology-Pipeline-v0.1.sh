...
usage_Help="Usage: ${0##*/} [-W run_hicup] [-Y run_bam2chicago] [-Z run_chicago] [-G run_genomescan]
              [-P run_plot] [-A bowtie2] [-X bowtie2_index_basename] [-d digest] [-H hicup_dir]
              [-C chicago_dir] [-I FASTQ_string] [-o out_dir] [-p output_prefix] [-b BED] [-R RMAP]
              [-B BAITMAP] [-D design_dir] [-O organism] [-r resolution] [-w scan_window] [-g gene_list]
              [-t threads] [-v] [-h] \n"
...
gene_list=$cwd"/utils/Charon_genes_v1.4.srt.bed" # Gene list in the capture panel for GenomeScan. Use -g to overwrite.
...
while getopts "A:B:b:C:d:D:g:G:hH:I:O:o:P:p:R:r:t:vw:W:X:Y:Z:" opt; do
    case $opt in
...
if [ "$run_genomescan" -eq 1 ]; then
    ...
    echo "bash $cwd/utils/genomescan.sh -i $hicup_output_bam_string -g $gene_list -w $scan_window -wf $window_file -p $output_prefix -o $out_genomescan &> $out_genomescan/$output_prefix/genomescan.log"
    bash $cwd"/utils/genomescan.sh" -i $hicup_output_bam_string -g $gene_list -w $scan_window -wf $window_file -p $output_prefix -o $out_genomescan &> $out_genomescan"/"$output_prefix"/genomescan.log"
    ...
    for d in $out_genomescan"/"$output_prefix"/gene_results/"$scan_window"/dir"*; do
        ...
        echo "Rscript $cwd/utils/significant_interactions.R $out_genomescan $currentDir $output_prefix $scan_window $padj_method $windows_genes $cwd/utils/ &> /dev/null &"
        Rscript $cwd"/utils/significant_interactions.R" $out_genomescan $currentDir $output_prefix $scan_window $padj_method $windows_genes $cwd"/utils/" &> "/dev/null" &

        echo "# Processing directory $currentDir in the background with job PID [$!] ..."
        pid[i++]=$!
    done

    echo -e "\n$threads background threads were submitted and are currently running ...\n"
    wait

    i=0
    for d in $out_genomescan"/"$output_prefix"/gene_results/"$scan_window"/dir"*; do
        currentDir=$( basename "$d" )
        echo "Job with PID ${pid[i++]} has finished! ($currentDir)"
    done

    # After all jobs are finished, concatenate the annotated results from all genes into one file
    cat $out_genomescan"/"$output_prefix"/significant_interactions/"$scan_window"/significant_events."*_annot.bed | grep -v 'chrStart' > $out_genomescan"/"$output_prefix"/significant_interactions/"$scan_window"/all_significant_windows.bed"
    numberofSignificantEvents=$(wc -l $out_genomescan"/"$output_prefix"/significant_interactions/"$scan_window"/all_significant_windows.bed" | cut -d' ' -f1)
    echo -e "\nGenomeScan identified $numberofSignificantEvents window(s) at $scan_window bp resolution across all $numberOfGenes genes of the probe panel.\n"

    timestamp=`date '+%Y/%m/%d %H:%M:%S'`
    echo -e "Finished running GenomeScan! [$timestamp]\n"
fi
